<?php
    ...
    if (isset($_GET['use_id'])) {
        ...
        $use = "UPDATE `shipment` SET `ship_status`='t' WHERE `ship_id` = '".$_GET['use_id']."'";
        $use_query = mysqli_query($con,$use) or die('cant update status');
       
    }
    if (isset($_GET['hold_id'])) {

        $use = "UPDATE `shipment` SET `ship_status`='f' WHERE `ship_id` = '".$_GET['hold_id']."'";
        $use_query = mysqli_query($con,$use) or die('cant update status');
    }
?>
                 ...
                 <div class="page-content">
                    <div class="container-fluid">
                          <div class="bg-white table-responsive rounded shadow-sm pt-3 pb-3 mb-30">
                              <h6 class="pl-3 pr-3 text-capitalize font400 mb-20">Manage Shipment</h6>
                            <table id="data-table" class="table mb-0 table-striped" cellspacing="0" width="100%">
                                <thead>
                                    ...
                                    <tbody>
                                        <?php
                                        $i=1;
                                        $p_order = "SELECT * FROM order_history,shipment,products,company WHERE order_history.u_id = $uid AND order_history.p_id = products.p_id AND order_history.company_name = company.c_id";
